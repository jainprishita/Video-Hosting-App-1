...
public class AuthenticationController {
    ...
    public ResponseEntity<AuthorizedUserResponse> login(@RequestHeader("authorization") final String authorization) throws AuthenticationFailedException {
            ...
            if(decodedArray.length<2)
                throw new AuthenticationFailedException("ATH-01", "Email Not found");
                UserAuthTokenEntity userAuthToken = authenticationService.authenticate(decodedArray[0], decodedArray[1]);
          //      System.out.println(decodedArray[0] + decodedArray[1]);
             if(userAuthToken==null)
                 throw new AuthenticationFailedException("ATH-02", "Wrong Password");
             UserEntity user = userAuthToken.getUser();
                ...
                System.out.println(userAuthToken.getAccessToken());
                headers.add("access-token", userAuthToken.getAccessToken());
                return new ResponseEntity<>(authorizedUserResponse, headers, HttpStatus.ACCEPTED);
